基本上，强类型的编译器，静态类型语言，例如 Pascal，在执行时期提供的可用类型信息很少，甚至没有。所有关于数据类型的信息只有在编译过程中可以被看的到。

Object Pascal 的第一版就打破了这个传统，它提供了属性与其他类成员的执行时期信息，只需透过一个特别的区段宣告关键词:published。这个类功能只要在编译器时有加上{$M+}这个编译设定即可，它同时也是 VCL 的DFM 档案串流化机制的基础(FireMonkey 函数库的 FMX 档案也是使用此一机制)，在窗体与其他可视化组件的设计环境也是使用此一机制。

在 Delphi 第一版推出的时候，这功能算是整个程序设计工具业界最创新的想法，后来许多程序开发工具也都用了各种不同的方法提供这个功能，并把这个功能加以延伸。

首先，在类型系统中有延伸功能来为方法进行注记跟检测(此功能仅有Object Pascal 语言提供)，以及在 COM 面动态进行调用。在 Object Pascal里面这功能仍然以 dispatch ID 进行支持，在方法中使用变异变量(Variant)以及其他跟 COM 相关的功能。最后，在 Object Pascal 里面对 COM 的支持已经由编程语言内建的动态时期类型信息来外延支持了，但这已经超过本书作为介绍编程语言功能的书该涵盖的范围了。

后来系统管理环境的出现，像是 Java 跟.NET，提出了一个很有延伸形式的执行时期类型信息(Run Time Type Information，以后简称 RTTI)，透过由编译器把详细的 RTTI 跟执行模块进行绑定，程序中就可以对这些模块检测与使用了。这有让内部源码跟模块的大小增加的副作用，但它也透过新的程序模块，以及结合动态编程语言的一些弹性到固定结构、增加强类型速度的优点。
...
+ 首先我们会先介绍新的延伸 RTTI，这功能已经内建在编译器，我们可以透过使用 RTTI 单元文件来使用里面的新功能与类。
...
Object Pascal 编译器默认就会建立许多延伸的 RTTI 信息。执行时期信息包含所有类型、包含类与所有其他使用者定义的类型，都会像是系统提供的核心数据类型一样提供相同层级的数据，并包含到私有区、保护区、公开区、发布区的所有元素。这需要深入到任何一个对象的所有内部结构。
...
这段源码使用了 TRttiContext 记录来参考 TButton 类型的信息，从这个类型的信息到 RTTI 关于属性的数据，以及这个属性的数据被用来参考到属性的内容值，它被转型成为了一个字符串。
...
技术上来说，这四个访问权限设定是使用以下宣告在 System 单元文件的集合类型来进行对应的:
...
系统中提供了一些预先定义好的常数值，可以和预设的 RTTI 内容对应，这些访问权限的可视范围是跟 TObject 的可视设定关连，所有的类也都继承了这些相同的权限：
...
编译器是否要建立这些信息，可以用一个新的编译器设定来处理：$RTTI，这当中包含了一个状态，用来指示特定类型或它的衍生类(明确指示或是继承)，并以三个特定的关键词来标明方法、数据字段或者属性的可视状态。在 System 单元文件中的默认值是:
...
要完整的关闭为所有类的成员建立延伸 RTTI 的话，我们可以使用以下的设定:
...
使用这个设定的时候，要记得它只会被套用在我们自己写的源码，而且不可能完全移除，因为用于 RTL 跟其他函数库的类的 RTTI 信息已经被编译到对应的 DCU 档案跟套件档案里面了。

同时也要记得，$RTTI 设定对于为已发布的类型所建立的传统 RTTI 的建立不会有任何影响：它还是会建立，跟是否设定$RTTI 无关。

!!! note "笔记"
    RTTI 处理类，是透过 RTTI 单元文件，我们稍后的章节会加以介绍，它是透过挂载在传统 RTTI 跟它的 PTypeInfo 结构来达成的。

我们可以透过这个设定值来停止为我们自定的类建立延伸 RTTI。相对的，我们也可以增加 RTTI 要建立的范围，包含私有区、保护区的方法跟属性，只要有需求的话(虽然建立这些信息不一定都有实质用处)。
...
当我们可以从执行档案里面取得 RTTI 信息，要注意到编译器加进去的信息，有时候链接程序会这些信息给移除掉。预设情形下，没有被编译到执行档的类跟方法就不会建立延伸 RTTI(因为在此一情形下，这些信息建了也没用)，因为也无法取得基础的 RTTI。

另一种情况下，如果我们想要让所有的延伸 RTTI 都被建立，并可以被使用，我们就需要把这些几乎没有用到的类跟方法进行连结。

我们可以使用两个编译器设定值来控制与执行文件链接的信息。第一个是$WeakLinkRTTI 设定值，它在文件里面有完整的叙述。设定这个值，在程序里面没有被使用到的类型以及该类型所属的 RTTI 都会被从执行档当中移除。

或者我们也可以强制把所有类型，以及这些类型的延伸 RTTI 透过$StrongLinkTypes 这个编译器设定值包进执行档里面。对很多程序来说，这个效果可是很戏剧化的，不过执行档的大小也大概会多出一倍来。


## RTTI 单元⽂件

如果为所有类型建立延伸 RTTI 是 Object Pascal 镜射功能的第一根柱子，第二根柱子是能够用简单、高阶的方式来浏览这些类型的能力(多亏了System.Rtti 单元文件)，第三根柱子我们待会会介绍，就是对于自定标注的支持，我们一个个来看吧。

传统的 Object Pascal 程序可以(现在还是可以喔)使用 TypeInfo 单元文件里面的函数来存取类『发布区』的执行时期类型信息。这个单元文件定义了一些低阶的数据结构跟函数(都是以指针跟记录为基础)，并定义了一些高阶的子程序来简化程序的运作。

不同的是，Rtti 单元文件则让使用延伸 RTTI 变得很容易，当中提供了一整组的类，包含了适当的方法跟属性。要存取不同的对象，进入点都是TRttiContext 记录结构，当中有四个方法用来搜寻可用的类型:
...
正如我们可以看到的，我们可以传递一个类，以一个从类型所取得的PTypeInfo 指标，一个完整的名称(包含从该类被宣告的单元文件开始到类型的完整名称，像是”System.TObject”)，或者取得类型的完整列表，被定义成一个 RTTI 类型的数组，或者更精确的说，是定义成 TArray<TRttiType>。我在以下的列表当中也有使用到最后的函数，是 TypesList 范例项目的版本简化:
...
GetTypes 方法回传了数据类型的完整列表，但程序会过滤掉内容，只留下类型所表示的相关类而已。在这个单元文件里面大概有十几个类所表示的类型。

_____________________________________________________________________
!!! note "笔记"
    RTTI 单元文件是把类类型当成”实体”或者”实体类型”(就像在TRttiInstanceType 里面)。这听起来有点容易混淆，就像我们平常把实际的对象称为实体一样。

在类型列表里面的独立对象是属于从 TRttiType 衍生而来的一些类。我们可以特别来看一下 TRttiInstanceType 类类型，把上面的源码重写成以下的程序片段:
...
这段源码会建立一个数以百计的数据类型列表，实际的数字要看编译器的版本跟其使用的平台了，我们可以看一下图 16.1。当中从 RTTI 单元文件列出了影像列表类型，我们会在下一节里面加以介绍。

图 16.1: TypeList 范例项目的执行结果


### 在 Rtti 单元文件里面的 RTTI 类

在以下的列表中，我们可以看到类继承关系的完整图表，这些类都是从 TRttiObject 衍生出来的，它们都定义在 Rtti 单元文件里面：
...
!!! note "笔记"
    在 Rtti 单元文件的第一次实作中，并没有 RTTI 对象可以存取被索引过的属性(像 TStringList 类的 Strings[]那样)。这是后来的版本才加上去的，现在仍旧可以使用，也让运行时间类型信息得以真正的完整。


### RTTI 对象的生命周期管理以及 TRttiContext 记录

如果看过了刚刚列出的 BtnTypesListClick 方法的原始码，里头有些地方看起来应该是相当严重的错误。GetTypes 调用的函数回传了一个类型的数组，但那段程序并没有把内部的对象是放掉。原因是 TRttiContext 记录结构变成了所有被建立出来的 RTTI 对象的拥有者。当这个记录被释放(亦即程序执行脱离了该函数)，一个内部接口会明确的调用它自己的解构函数来是放掉被建立出来的所有 RTTI 对象。
...
内部作业里，TRttiContext 记录使用了 TRttiPool 类型的一个全局缓冲区，它使用了临界区(Critical section, 这个名词我很习惯直接用英文，所以用这个翻译字眼不知道恰不恰当)来保证多线程的作业安全。

!!! note "笔记"
    在 RTTI 的缓冲机制里面也会发生多线程同时存取的意外，相关的信息就直接被写在 Rtti 单元文件里面的注释文字。

所以，精确的说，RTTI 缓冲区是让所有的 TRttiContext 记录所共享的，因此被保留着的 RTTI 对象会一直被维持着，直到所有内存里面的TRttiContext 记录都被释放掉为止。让我引用一下该单元文件里面的注释:
...
虚构的建构函数跟虚构的解构函数会设定内部的接口，这些内部接口则会管理实际在背景使用到的数据结构，并把它设定成 nil 来清除这些缓冲区。然而，这个动作会对于区域类型，像是记录自动执行，这个动作不用特别处理，除非我们用了指标来参照这些记录。

### 显示类信息

我们在运行时间中最想要进行检查的相关类型，当然是结构化的类型，例如接口或记录。聚焦在这些实例上，我们可以参照这些类之间的关系，透过以下可以用在实例类型上的 BaseType 信息。

存取类型当然是一个很有趣的开始，但相对的也是具备认识这些类型的内容的能力中相对比较新的，包含它们的成员。当我们点击这些类型中的任何一个(我们在范例中使用了 TPopup 组件类)，程序就会显示一连串的属性、方法，以及该类型的数据字段，我们透过三个分页加以呈现，请见图16.2。

图 16.2: 在TypesList 范例项目中所显示的详细类型信息


第二个窗体的单元文件，可以用以延伸成为一个用在其他应用程序中通用的的类型浏览器，当中有一个名为 ShowTypeInformation 的方法，可以浏览特定类型当中的每一个属性、方法，以及数据字段，浏览的方式则是把它们加入到三个独立的列表，各自以不同的可视权限作为每个列表显示的内容(pri 会显示私有区，pro 会显示保护区，pub 则会显示公开区，pbl 则是显示发布区，这个显示的判别是写在 VisibilityToken 函数里面):
...
我们可以继续解析这些属性所属的类型，来找出更深入的信息，取得该方法的参数列表，并检测回传类型等等。这个范例并不是要建立出完整的RTTI 浏览器，只是要让我们知道，我们可以透过 RTTI 的功能获得哪些信息。


### 套件中的 RTTI

除了我们可以用来存取某个类型或者一系列类型的方法，TRttiContext 记录也提供了另外一个很有趣的方法”GetPackages”，这个方法可以回传目前应用程序有使用到的运行时间套件列表。如果我们在一个没有使用任何运行时间套件应用程序里面调用这个方法，回传值的内容就只会有目前的执行档而已。但如果我们在一个使用了许多运行时间套件的应用程序里面调用它，回传值的内容就会是这些套件的清单。从这个信息，我们就可以深入到每个套件里可以使用的类型。请留意到这个案例中，类型的清单可能会非常冗长，因为 RTL 跟视觉组件函数库当中没有被使用到的类型并不会被链接程序自动排除。

如果我们使用了运行时间套件，我们也可以取得每个套件当中所有类型的清单(也包含执行文件自己)，我们可以使用以下的源码:
...
新的延伸 RTTI 不只让我们可以对程序内部结构进行浏览，还提供了特定的信 息 ， 包含属 性 和 数 据 字 段 值 。 在 TypInfo 单 元 文 件 提 供 了 名 为GetPropValue 的函数可以存取一般的属性，并以当中的值存取其对应的变异类型，新的 Rtti 单元文件则使用了不同的结构来处理这一类没有特别定义类型的元素，这个结构称为 TValue 记录。

这个记录几乎可以储存 Object Pascal 里面所有的数据类型，并且可以追踪它的原始数据表示方式，它的作法则是同时记录数据与其数据类型。因此我们就可以对该变量指定的类型进行读写。如果我们把整数写进 TValue，我们读出的就只能是整数类型了，如果写进去的是字符串的话，读出来的也就会是字符串。(在 XE6 之后，FireMonkey 里面的 TGrid 就已经使用TValue 来设定每个 Cell 的内容，所以不像以往有分成 TStringGrid 等不同类型，TGrid 就涵盖了所有类型的 Grid)。

但 TValue 不能提供不同类型之间的格式转换，因此就算 TValue 有提供AsString 与 AsInteger 方法，我们也只能在 TValue 的内容的确是字符串时使用 AsString，而在 TValue 内容的确是整数的时候才能使用 AsInteger。举例来说，在以下的案例里，我们可以使用 AsInteger 方法，如果我们调用IsOrdinal 方法的话，回传值就会是 True:
...
不过，如果我们需要使用字符串来表示 TValue 的内容，则可以用 ToString方法，这个方法里面有一个 case 判别逻辑，可以把大多数数据类型的内容转换成字符串：
...
> TValue 是个类型，可以用来存取以 RTTI 为基础的方法，并且读写相关的数据字段与属性。

> 这个类型跟变异类型(Variant)类似，但其工作原理更偏向 Object Pascal 的类型系统。举例来说，实体可以被直接储存，像是集合(set)、类参考等等。它也是更具局限性的类型，并且不会(例如)在背景进行字符串对数字的转换。

现在我们对 TValue 的角色应该有更深一层的理解了，让我们来看一下TValue 记录的实际能耐吧。它具备一组高阶的方法，可以指派、解析出实际的内容，也包含一组低阶的以指标为基础的方法。我们接下来会集中篇幅在高阶方法上。

在指派内容时，TValue 定义了许多个 Implict 运算方法，让我们可以直接在源码里面把特定的值指派给 TValue 类型的变数：
...
这些运算方法所做的，就只是调用泛型类方法 From 而已:

`class function From<T>(const Value: T): TValue; static;`

当我们调用这些类函数的时候，我们需要确定数据类型，并传递一个该类型的数据过去，像是以下的源码，会透过上述的这些类方法，把整数值 100 进行指派:

`V1 := TValue.From<Integer>(100);`

这是一系列的统一技术，用来把所有类型的数据搬移到 TValue 类型的变数里去。一旦资料被指派完成，我们就可以用以下的几个方法来检测该数据的实际类型了:
...
请注意这里面的 IsType 泛型方法，几乎可以用在任何数据类型上。对于解析出来的数据，也有对应的方法，但我们只能对 TValue 里面储存的数据使用类型兼容的方法，因为当中并不会有任何转换的动作发生：
...
这些方法大多会同时有 Try 的版本，会在执行时，如果发现类型不兼容时回传 False，而不会触发例外事件。当中也有一些限制转换的方法，最相关的是泛型函数 Cast 与 ToString 函数，我已经在范例中使用过它们了:
...
TValue 的重要性，来自于这个结构会在存取属性或数据字段时，使用延伸的 RTTI 与 Rtti 单元文件。作为使用 TValue 的实际范例，我们可以用这个记录类型来存取 TButton 对象发布区的属性与私有区的数据字段，请参考以下的代码段(节录自 RttiAccess 范例项目):
...
作为更好的替代方案，在 TRttiMethod 类中就定义了一个简化版的 Invoke多载版本：
...
标注，用 Object Pascal 或 C#的术语来说叫标注，用 Java 的习惯来看叫注释，是我们可以在源代码里面加入的说明或指示，我们可以把它加入到类型、数据字段、方法、或者属性(Property)，编译器会把它跟产出的执行二进制源码包在一起。这是通常用方括号进行的指示:
...
通常标注不是用来改变类或对象的实际核心功能，而是让这些类能够在参与运作时能够有延伸特定功能的机制。把一个类宣告为可串行化(serializable)，并不会影响到类的任何功能，但会让串行化功能的源码得知这个类需要被串行化处理，以及该如何处理(在这个例子里面，我们需要透过标注来提供更多信息，或者在类的数据字段或属性中建立更进一步的标注)。
...
举例来说，『员工』这个类仍旧是从原有的架构上，一定还是从个人的类中所衍生出来的，『员工』这个对象还是会对该对象需要提供一个标识符的字段，但我们可以”标注”这个『员工』类需要跟数据库的特定数据表进行对应，或者跟特定的运行时间窗体进行对应。所以我们可以透过继承(是特定类)、拥有关系(属于)，以及标注(标示为)作为三种独立的机制，我们可以在设计应用程序的时候，这三种机制都可以是需求来用上。
...
我们要如何定义一个新的标注类(或者标注分类)呢?我们必须从 System单元文件里面的一个新类: TCustomAttribute 类来衍生出新的类：
...
我们为标注类取的名字会变成在源码里面使用的标识符，我们可以选择把标注加在后面作为延伸。所以如果我们为类命名为 SimpleAttribute，我们可以在源码里面用 Simple 或者 SimpleAttribute 来使用这个类。因此Object Pascal 里面不成文的规定，为每个类都以 T 开头来命名的这个规定 ， 通 常 在使用 标 注 的 时 候 ，就不使用了，主 要 的 例 外 会 是System.TCustomAttriute。
...
在上例中，我使用了 Simple 这个标注加在整个类跟一个方法上面。除了名字，标注也支持一个或一个以上的参数。传递给标注的参数必须跟该标注类的建构函数声明相同，如果该标注类有建构函数的话:
...
要被传递给该类的标注的值，必须是一个常数或表达式，因为常数或表达式的内容在编译阶段就可以被解译出来了。这也是只有少数的数据类型:有序内容、字符串、集合，以及类参考可以使用的原因。从正面来看，我们可以用不同的参数来建立多种覆写版本的建构函数。注意一下，我们可以把许多个标注只在同一个标识符上面，就像在 RttiAtrrib 范例项目里面所做的，我们整理一下在这一节里面的代码段:
...
最后，跟其他类似的概念的实作方式做个比较，在标注的使用上目前机会没有方法可以限制使用的范围，像是在宣告区段，标注可以用在类型上，但不能用在方法上。然而在编辑器上面，标注在使用更名重购(rename refactoring)的功能上几乎是完全可以使用，没有任何限制的。我们不只可以更改标注类的名字，系统还会自动在该标注被使用在完整的名字，以及没有使用最终完整标注的地方帮我们补正。
...
现在如果没有方法可以找到哪些标注是有被定义过的，这些源码看起来就完全没有用了，而且可能因为这些标注，在对象当中注入不同的规则或行为。我们先来介绍第一部分吧。在 Rtti 单元中的类，让我们可以厘清哪些标识符有相关的标注。

以下的源码是从 RttiAttrib 范例项目中节录出来的，可以显示出跟目前的类有关连的标注清单：
...
我们可以在源码的 for-in 循环里面做些修改，来解析出特定的标注类型：
...
我们一般所做的并不是简单的描述标注，而是对特定的标注类加上一些独立的行为，跟它原本的源码不同。举个实例，我们可以在上面的源码里面注入特定行为: 目标是调用有特定标注类的所有方法，先考虑它们都是不需要参数的方法:
...
这个程序片段所做的动作包含了建立一个对象、抓出该对象的类型，检测特定的标注，最后调用每个有标注为 Simple 的方法。这跟从类继承、实作接口、或者写一些特定的源码来行使这个要求都不同，要取得新的动作，我们只需要以特定的标注来为一个或多个方法进行加注。这并不式说这个例子使得标注的使用异常明显: 我们可以在本章的最后参照一些常见的使用标注的模式，和一些实际的个案研究。
...
有另一个相关的功能在延伸 RTTI 被加入 Object Pascal 之后也被加入了，这个功能是对一个现存的类可以透过虚拟方法来加以拦截的能力，只要对现存的对象建立一个代理类。换句话说，我们可以选一个已经存在的对象，然后修改它的虚拟方法(可以一次只修改其中一个，或一次把全部都处理掉)。

我们要怎么完成这一点?在标准的 Object Pascal 应用程序中，我们应该没办法使用这个功能。如果我们需要让一个对象拥有不同的行为，就只能先衍生一个子类，然后在子类里面修改源码来达成。而在函数库里面则是完全不同的，因为函数库必须用很通用的方式来撰写，对对象的了解不多，函数库可以操作，并且尽可能的降低对对象的负担。这就是在 Object Pascal里面加入虚拟方法拦截器的情境。

!!! note "笔记"
    我们可以从一则部落格的发文看到对于虚拟方法拦截器(这部份我是比较欠缺的): http://blog.barrkel.com/2010/09/virtual-method-interception.html。

在我们聚焦在可能的情境之前，我们先讨论技术本身吧。假设我们有一个已经存在的类，它带有至少一个虚拟方法，像下面的例子这样:
...
假设我们在这个类里有一个叫 FPerson1 的对象。现在，我们可以做的就是建立一个TVirtualMethodInterceptor对象(它是定义在Rtti单元文件里面的一个新类)把它跟我们想要制作的新类(TPerson)绑定起来，成为一个子类的对象，把 FPerson1 对象的固定类变成动态类:
...
在 InterceptBaseClass 范例中，范例中使用了上面的 TPerson 类，我拦截了类的虚拟方法，加入了以下的纪录源码:
...
注意到，FVmi 对象需要被维持到 FPerson1 对象被使用结束为止，不然我们就会使用一个动态类，而该类已经不能用了，最后我们就会调用到一个已经被释放掉的匿名方法。在范例中，我把它存放在一个窗体的数据字段中，就跟对象会参考到的数据(FPerson1)一样。


这个程序会透过调用它的方法以及检测基础类的名称来使用对象:
...
请注意这个类跟基础类的名称完全一样，但事实上却是完全不同的两个类，它是用虚拟方法拦截器所制作的动态类。

纵使并没有官方的作法可以把目标对象的类恢复成原始的类，该类本身仍旧可以透过虚拟方法拦截器对象运作，也仍旧是该对象的基础类。当然我们也可以硬把正确的类参考硬塞给该对象(初始时是四个 Bytes)的类数据:

`PPointer(Person1)^ := FVmi.OriginalClass;`

举个更难的例子，我们已经修改了 OnBefore 的源码，所以在这个案例中，如果我们调用了 Age，它会回传特定的值，而把原来类源码中的动作给忽略掉:
...
第一个范例程序，会展现使用标注来识别在类中特定的信息。具体来说，我们希望可以解析一个对象，该对象的类是属于类架构中的一员，并且拥有说明与独特的代号，这个代号可用来参照到该对象。这可以用来处理几种情形，像是描述储存在集合组件(可能是泛型集合对象或是传统的集合对象)当中的对象。
...
如果我们想要有一些方法可以让许多对象共享，最传统的作法就是定义一个具备虚拟方法的类，然后用这个类作为基础类来衍生几个不同的对象，并覆写这些虚拟方法。这个方法不错，但是在类上还是有不少限制，这些限制也会由于类结构而产生，因为我们让这些对象拥有同一个基础类了。

完全不同的风格(当然有优点也有缺点)是使用标注来为特定的类、方法货属性进行标示。这个作法比较有弹性，而且并没有使用到接口，而是基于一个相对比较慢，且容易出错的执行时期信息搜寻，和编译时期的解决方法完全不同。这表示我们不崇尚这种以接口作为更高目标的程序风格，而只是作为一个可能值得评估，以及在某些情形下使用的话可能会很有趣的作法。

**描述标注类(The Description Attribute Class)**

为了这个范例，我已经定义了会被应用到的标注，作为可以被搜寻的元素。我们可以使用三个不同的标注，但应该要避免污染命名空间的标注。以下是标注类的定义：
...
接下来，我写了两个使用标注的简单类。每个类都用标注来标示，并且有两个方法以相同的标注，但不同的自定方式来标示。
...
第二个类(TCompany)相对的更简单，因为它有自己的代号跟描述:
...
虽然这两个类有点相似，但他们在类架构上、一般接口、或是任何面向都是完全不相干的。它们只共享了相同名字的标注而已。
...
要显示跟这个对象相关的信息(通常是有被命名的代号跟描述，如果有定义的话)，在程序中使用了透过 RTTI 功能进行的标注寻找功能。首先，使用了助手函数来判断该类是否有被以特定的标注进行标示：
...
!!! note "笔记"
    在这个案例里面，我们需要检查完整的类名称，DescriptionAttribute，则不只检查『描述』内容，它是当编译器透过方括号来辨识短名称时，我们要套用标注时可以使用的标识符。
...
如果这个程序产生了不太有趣的输出结果，那么执行的方法一定会是相关的，因为我把一些类注释掉了，其中这些类的两个方法又使用了同一个标注，且已经让这些类可以用外部的算法来进行处理。

换句话说，这些类并不需要特定的基础类，不用实作接口，也不用任何类架构中提供任何内部源码，只需要这些类在宣告的时候记得使用标注就行了。而管理这些类的完全责任，则落在了外部的源码身上。
...
为了协助建立这样的 XML，我已经以让 XmlPersist 范例以一个延伸版的TTrivialXmlEriter 为基础，这个类原本是我在撰写 Delphi 2009 Handbook这本书的时候，为了介绍 TTextWriter 类所做的范例。在这里我就不再对它多做赘述了。我只想说，这个类可以持续追踪它所开启的 XML 节点(感谢字符串堆栈)，然后依照后进先出的顺序关闭 XML 节点。


!!! note "笔记"
    TTrivialXmlWriter 类的原始码，可以从 Delphi 2009 Handbook 的范例找到，网址是:http://github.com/MarcoDelphiBooks/Delphi2009Handbook/tree/master/07 /ReaderWriter

我在原始的类中加入了一些限制格式的源码，以及用来储存对象的三个方法，基于我们在本节里面要介绍的三个不同作法，以下是类的完整宣告内容:
...
在介绍过涵盖了所支持的类之后，我们从基础开始吧，也就是把一个对象用传统 RTTI 将发布区的属性以 XML 为基础格式进行储存。
...
这段源码的功能，是要求一定数量的属性，配置适当大小的空间给数据结构使用，并把发布区属性的信息填入这些数据结构。如果我们想知道是否可以用低阶源码来做这些事情呢?发出这个问题的同时，我们就知道为什么新版的 RTTI 要被发展出来了：就是为了简化、隐藏旧版 RTTI 的复杂。为了每一个字段，程序会把数字跟字符串类型的属性内容解析出来，如果解析出来的是任何子对象的话，也会进一步解析这个子对象:
...
为了示范这个程序的效果，我已经写了两个类(TCompany 跟 TPerson)，这两个类是从前一个范例借过来的。然而现在 Company 对象有了一个额外的 Person 类型属性，叫做 Boss。在实际的世界，这可能很复杂，但在这个范例中，这是很合理的假设。以下是这两个类的发布区属性宣告:
...
这个程序的主窗体有一个按钮，用来建立并连结这两个类建立出来的两个对象，并且把它们储存到一个 XML 串流中，稍后我们会让它显示出来。以下是串流化的源码:
...
然而另一个更大的不同，是在这个案例中，类不需要从 TPersistent 类衍生而来，也不需要有任何特别的限制。
...
这些是我们可以用标注来加以厘清的问题，但它的缺点是在宣告类的时候大量使用标注，这种风格我并不喜欢。对于新版的源码，我已经透过一个选用的参数定义了一个标注化的建构函数：
...
没有 XML 标注的数据字段会被忽略掉，在 XML 输出的标签是可以自定的。为了示范这一点，程序中有以下的类(这一次我已经略过了发布区的属性，因为它们没相关):
