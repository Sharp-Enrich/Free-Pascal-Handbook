...
如同类别可以有自定的建构函式，类别也可以拥有自定的结构函式，以destructor 为名宣告的方法，呼叫时直接呼叫 Destroy 即可。这个解构函式可以在对象被从内存毁灭之前，先把使用过的资源清除掉，但大多数的情形下，并不需要自定的解构函式。
...
有了这两个建构函式，我们就可以用以下两种方法来建立新的 TDate 对象了：
