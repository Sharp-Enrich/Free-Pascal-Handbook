...
两种写法的子程序都可以传递多个相同或不同类型的参数，我们稍后会介绍，程序跟函数都是类别(Class)当中用来实作方法(method)的基础，而在这个案例中，两种形式的区别仍旧存在。事实上，跟 C, C++, JAVA, C#或JavaScript 都不同，我们在宣告函数或者方法的时候，仍旧会用到 procedure或者 function 这两个关键词来宣告子程序或者方法。
...
我们已经介绍了要怎么在单元文件的 interface 区段透过 forward 关键词进行预先宣告。在类别的类型当中宣告方法，也可以视为一种预先宣告。
...
跟传统的传值参数规则比较一下，引用参数所传递的不只是参数里面的数值，而是把整个参数传过去，所以引用参数不能接受把常数、表达式、函数的回传值，或是类别的属性当成引用参数传递。另一个规则则是不能传递不同类型的变量(必须要先经过转型)。变量的类型跟参数的类型必须完全一致，不然编译器又要抱怨有错了：
