...
在 Object Pascal 里面，子程序以两种形式存在：程序(procedure)和函数(function)。理论上，程序是我们要求计算机执行的一个动作，而函数是计算后要把数据回传的动作。两者之间的差异，是函数会回传结果，这个回传的结果可能是数值、型别，而程序不会回传任何数据。在 C 语言里面，只提供了单一语法，就是函数，当不回传数据的时候，C 语言就要求程序人员把回传数据的型别写成 void，C 语言的这个作法就完全等同于 Object Pascal 的程序。


两种写法的子程序都可以传递多个相同或不同型别的参数，我们稍后会介绍，程序跟函数都是类别(Class)当中用来实作方法(method)的基础，而在这个案例中，两种形式的区别仍旧存在。事实上，跟 C, C++, JAVA, C#或JavaScript 都不同，我们在宣告函数或者方法的时候，仍旧会用到 procedure或者 function 这两个关键词来宣告子程序或者方法。
...
事实上 C 语言的语法在撰写程序跟函数时完全相同。而在 Pascal 语言的语法中，函数还需要一个特殊的关键词与回传值(或者回传的型别)。


!!! note "笔记"
    在 Object Pascal 的语法中，还有一个地方跟其他语言不同的，就是在函数声明的时候，结尾处必须用冒号来宣告函数要回传的数据型别。
...
当我们需要使用一个识别符号(不论其型别是什么)，编译器必须预先得知这个符号，并且必须知道这个识别符号将会指向的参考地址。为了满足这个需求，我们得在使用任何识别符号之前先提供完整的定义。然而，有些情况下这个要求很难达成。如果程序 A 调用程序 B，而程序 B 又调用程序 A，当我们开始撰写这个源码的时候，我们等于是在调用一个编译器还没看到的程序。
...
!!! note "笔记"
    范例程序里面的两种次方函数都没有处理负次方的能力，如果把次方的参数传个负数进去，就会造成无穷循环了。同时因为使用了整数型别，也会很快达到该型别的最大值，并造成数值溢位。我撰写这些源码时，也同时保留了这些先天的限制，好让源码能保持简洁
...
但『方法』到底是什么？方法，是程序或函数与记录或类别相关的特别型态。在 Object Pascal 里面，每当我们为视觉组件处理事件的时候，我们就需要定义一个『方法』，通常是一个程序，但『方法』这个名词，是用来指属于特定类别(或记录)的函数或程序，所以同时包含两者。
...
当我们调用函数或者程序的时候，我们必须传递正确数目的参数，并且必须确认所有参数的型别都跟宣告的相同。要不然，编译器就会指出这里有错误，例如参数型别不符。以前面的定义的 DoubleIt 函数当例子，它是要求整数作为参数，所以如果我们这样调用：
...
跟传统的传值参数规则比较一下，引用参数所传递的不只是参数里面的数值，而是把整个参数传过去，所以引用参数不能接受把常数、表达式、函数的回传值，或是类别的属性当成引用参数传递。另一个规则则是不能传递不同型别的变量(必须要先经过转型)。变量的型别跟参数的型别必须完全一致，不然编译器又要抱怨有错了：
...
除了标准的引用参数型别(var)之外，Object Pascal 还提供另外一种很特别的参数关键词: out。out 参数不用具备初始值，它只用来把数据回传给调用者，除了不用提供初始值以外，其他的规则，out 跟 var 几乎都一样。
...
常数参数常被用在字符串型别上，因为在这种情形下，编译器会先暂时停止参考计算器制(Reference Counting mechanism)，以进行优化。使用常数参数最常见的原因，就是常数可以在使用有序型别时可以产生一定的界限。常数参数也不常被用在对象传递上，因为在 Object Pascal 里，如果我们用常数参数形式传递对象的话，被传递进去的会是该对象的参考(内存地址)，而非对象本身。换句话说，编译器不会让我们指派另一个对象到该参数中，但可以让我们调用该对象当中的方法来改变对象里面的数据。
...
+ 在多载的每个函数(或程序)宣告中，参数的数量或型别必须要有不同，参数名称则无所谓，因为名称在调用过程中不会被特别指出，而回传值
并不会被拿来作为两个多载函数的判别，换句话说，多载函数(或程序)当然可以回传相同型别的数据。
...
!!! tip "提示"
    IDE 的 Code Parameters 技术对多载的程序与函数处理的非常好。当我们在编辑器上面输入了函数名称，且输入了小括号，所有同名的多载函数就会一起被自动列出。当我们输入参数的时候，Code Insight 技术则会依照我们输入的参数型别自动判别符合的多载函数，隐藏其他不符型别的。所以在撰写源码的时候就轻松多了。
...
然而，我们可以在别的单元文件里面建立一个新的函数，然后把这个新单元文件当成命名空间来处理。在这个情形下，我们就不是新增一个多载版本函数，而是把原来的函数用一个完全新版的同名函数来取代了，原来的函数会被编译器视为被隐藏了(当然还是可以直接把该单元文件的名称作为前述字符串，来调用原始版本的函数)，这也就是为什么编译器不能只从参数来判断，就决定用哪一个多载版本，但编译器也会试着用唯一相符的多载版本函数来处理，但仍会在参数型别不相符的时候发出错误讯息。
...
但还有第三种情况：假设编译器能够对函数的参数进行型别转换，则对单一个函数调用就有可能有不同的转换。当编译器发现它可以使用的多种多载版本函数，而其中并不包含完全相符的参数型别，则此时编译器发出的错误讯息就会是『此函数调用发生混淆』。
...
然而，事实上通常一个函数可以在参数经过型别转换后接受该参数，例如一个函数要求浮点数型别，但可以在整数资料经过型别转换以后接受它，所以我们可以写成：
...
如果我们在实际上写程序的时候遇到上面这个情形，而我们真的需要调用这个函数，我们可以自己先做型别转换来解决这个问题，并让编译器可以判别我们要调用的是哪一个多载版本：

`Show (Add (10, 10.ToSingle).ToString);`

实际会发生混淆调用的情形，会是我们使用 variant 这种型别作为参数的时候，因为这个参数型别可以包含许多种不同特定型别，我们在本书稍后的篇幅会加以讨论。
...
其中一个限制，是这个函数或方法不能参考到任何定义在同一个单元文件的implementation 区段的识别符号(例如型别、全局变量或函数)，而他们在调用的源码中也不能被使用到。然而，如果我们调用的是一个区域函数，则内嵌这个函数就不会有任何问题了，编译器会遵照我们的要求，把这个函数进行内嵌。
...
我们需要使用到不同于预设调用约定的情形，就是当我们需要使用到一个操作系统的原生 API 时，这时我们会被要求使用该平台专属的调用约定。即使是 Win64，使用的调用约定也跟 Win32 不同，所以 Object Pascal 支持很多不同的选项，在这里我们先不详述。而行动作业平台常常愿意揭露类别，而不揭露原生函数。即使在这种情形下，我们也必须考虑到尊重该平台的调用约定的问题。
...
在 Object Pascal 里(不是在传统的 Pascal 语言里面喔)，存在着程序型别的概念(这个概念像极了 C 语言里面函数指针的概念-在 C#跟 JAVA 里面都舍弃了这个功能，因为这个功能会和全局变量紧密结合)。

程序型别的宣告会需要指定一连串的参数以及回传值来定义函数。举个例子，我们可以宣告一个新的程序型别，需要一个整数参数以传址形式传入：

`type TIntProc = procedure (var Num: Integer);`

这个程序型别兼容于具备完全符合的参数的程序(用 C 语言的用语来说，就是函数特征相同)，以下就是一个兼容于前述型别的程序：
...
程序型别通常有两个要求：我们可以以程序类型声明变量，或者把程序型别当成参数型别传给另一个函数。假设前述的型别跟程序宣告属实，我们可以写下这些源码：
...
虽然没有直接调用这些函数，其中一些已经被存成程序型别的变量了。这个变量会在用户选择一个核取方框(checkbox)时有一些改变，目前的程序是以传统方式仿真用户点击按钮。这些程序使用了两个全局变量(被调用的函数，以及当前的现值)，所以这些数值都会被经年累月的保留下来。以下是完整的源码，请以真实程序的完整定义把这些数据封装起来吧：
...
另一个使用到程序型别的实例，是当我们要把一个函数传递给操作系统时，例如 Windows(通常在操作系统中都有所谓的 callback 函数)。在这个章节要开始的时候有提到过，Delphi 程序人员使用方法指针来处理程序化型别(我们会在第 10 章介绍)以及匿名方法(我们在第 15 章加以介绍)：
...
动态加载提供了很大的弹性，我们得自己处理加载函数库的程序，使用的是GetProcAddress API，来找到我们想调用的函数，然后把该函数的指标转换成适当的型别之后就能调用它了。这样的源码是相当繁复且容易出错的。
