...
我已经透过类别自动完整的功能，在 TFormAnonymFirst 窗体类别(属于AnonymFirst 范例项目)中加入了一个属性，其类别是匿名方法指标类型(嗯…实际上我已经在该项目中使用了相同的匿名方法指标类型了):
...
我们再来看另一个范例(AnonButton)，我已经在范例中的按钮上加入了一个匿名的点击事件，宣告一个适当的方法指标类型，并为一个自定的按钮类别(使用中介类别定义)加入一个新的事件：
