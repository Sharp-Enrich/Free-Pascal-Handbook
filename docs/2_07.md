...
!!! note "笔记"
    您可能会觉得奇怪，为什么当对象参考是 nil 的时候，还可以安全的调用它的 Free 方法，但我们却不能直接调用 Destroy 方法?理由是 Free 方法是位于已知的内存空间，而虚拟函数 Destroy 是在运行时间依照该对象的型别进行搜寻以后决定的，搜寻的结果有可能会是对象不存在，那这个动作就非常危险了。
...
透过这个函数，在前一个 SafeCode 范例项目，我们可以在进行安全型别转换前加上一个指针-到-对象的检查：
...
相同的概念也可以直接应用在类别参考上，也可以用来实作类别参考之间的安全型别转换。而且，最好能够一开始就写好安全、明白的源码，试着避免类似的问题，但如果我们无法避免，IsPointerToObject 这个函数就有机会派上用场了。无论如何，这一节的用意应该是在对于这些系统数据结构的内容多做一些说明。