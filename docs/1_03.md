...
如果我们要判断的条件很复杂，例如对同一个变量的数值要分成多个不同部分来分区处理的时候，用 IF 叙述句会变得很复杂，这时候我们可以改用case 叙述句。Case 叙述句可以让我们判断同一个表达式的不同范围的数值，这些数值必须是常数，而且必须是有序类型的数值，不能重复。最后，我们也可以在所有列举的范围数值之外，使用 else 叙述句，让我们没能预想到的所有情形都在该段源码来处理。Case 叙述句并没有像 endcase 这样的结束关键词，它仍旧使用 end 来结尾。
