...
编程语言的语句是以关键词为基础，搭配其他元素，让我们能够让编译器得知我们要执行的一系列程序。语句通常会被以程序或函数的方式包装起来，这一点我们会在下个章节介绍，而目前我们只要先聚焦在我们可以用来撰写程序的一些基本的指令即可。我们在第一章里面介绍过的（在介绍使用空格符与源码样式的篇幅里）实际上撰写程序是很自由的，我们也介绍了批注与一些特别的元素，但还没有来得及完整的介绍其他核心概念，像是程序的语句。
