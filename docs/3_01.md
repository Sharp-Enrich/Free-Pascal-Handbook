Object Pascal 所提供的强类型检查对于提升源码的正确性是很有效的，这也是我在本书中不断提到的。强类型的检查有时候也很烦，就像有时候我们想要写一个程序，让这个程序可以处理不同数据类型或类。这个议题已经被 Object Pascal 纳入成为功能之一，就像 C#, Java 这些相似的语言一样，我们称之为泛型(Generics)。

泛型的概念，或者用 C++的名词来说，叫做类样板(template classes)。我在 1994 年所著，关于 C++的书里面有写道:

> 我们可以宣告一个类，其中的一个或多个数据成员不特别指定使用哪种类型：这个类型的指定可以延迟到当我们需要定义该类的变量时才来决定。我们在定义函数的时候也可以用相似的作法，不在撰写的函数的时候定义一个或多个参数的类型，直到该函数要被调用的时候才来指定。
...
本章的第一个范例就用一个很常见的类，我实作了一个键-值对的数据结构。以下第一个程序片段，就是用传统的写法来建立这个数据结构，透过一个对象来储存数据的数值：
...
要使用这个类，我们可以建立一件对象，设定它的键与值，然后使用它，就像下列的源码，节录自 KeyValueClassic 范例项目的主窗体中的几个方法：
...
要是我们需要一个类似的类，储存的是整数，而不是对象呢?好吧，我们也可以自己把整数作强制类型转换(这么做，出错的风险很高的)，或者建立一个新的类，储存字符串键以及整数值。或许直接剪贴源码，然后修改里面的一部分也是种解决方法，最后我们就会有两段源码基本上完全一样，这样做，我们就抵触了良好程序设计的原则之一，而且我们以后如果发现源码里面有问题，就得同时改两次源码，如果复制三次，就要改三次，如果复制 20 次呢…… 那就烦死了。

泛型让我们可以对上例中的值的定义变得比较有弹性，而只要写一个泛型类即可。一旦我们对键-值泛型类进行了初始化，它就变成了特定的类，直接跟特定的类型进行连结了。所以我们可以透过同样的作法，在我们的应用程序里面让这个类宣告分别跟两种、三种，甚至 20 种类型进行连结，但类的源码还是同一份，只是在变量或者属性宣告的时候，指定这个类要跟哪一种数据类型进行链接。这样就不会在执行时期产生多余的动作了。让我们接着做，开始为我们成对的键-值，定义一个泛型类：
...
在上述的类定义里，有一个尚未明确定义的类型，以字母 T 来代表。字母 T 在很多语言中都被使用来当做泛型类中的符号，但我们可以在源码中用任何我们喜欢的字眼来代表泛型的类或类型。使用 T 这个字母，是为了增加程序的可读性，泛型类只是用这个符号来表示这个符号出现的地方，未来会以参数指定的类型来进行替换。如果类中要进行泛型处理的地方有很多个，我们在实务上就会用它的角色来为这个代号进行命名，不用多个字母来代表(T, U, V)，这种方法是早期 C++里面的写法。

!!! note "笔记"
    T 已经是通用的不成文写法，因为 C++语言在 1990 年代前期提出样板(template)这个概念的时候，就直接以 T 来表示样板。当时作者是要用 T 来表示样板(template)还是类型(Type)，目前已不可考。这样的转换在 Delphi的世界里运作完美，类型只要以 T 字母开头即可，用 T 来表示 Type 是很有意义的作法。

泛型类 TKeyValue<T>使用了尚未明确定义的类型作为它储存的两个数据字段之一、属性的值，以及设定方法(setter)的参数。方法的源码则很平常，但要注意到这当中也是使用了泛型类型，它们的定义包含了该类的完整名称，包含泛型类型：
...
然而要使用这个类时，我们就得完整的写出真正的类型了，必须提供实际的数据类型作为泛型类型。例如我们可以宣告一个键-值对象把按钮作为类中储存的值，可以这样写:
...
在建立实体的时候，也需要写出完整的名字，因为这已经是确切的类型了(泛型类型中尚未实体化的类型名称看起来则像是一个类型建构机制)。

在键-值对当中使用尚未明确定义的类型来定义值的字段，会使得源码更能有弹性，我们现在只用了 TButton 来当做储存的值的字段，当然 TButton的所有衍生类对象也都可以存在这里面，我们也可以在解析出值字段的对象之后，使用该对象的各种方法。
...
在前一版的源码里面，要指派泛型对象的时候，我们可以指派一个按钮或者窗体，但现在只能储存按钮了，这也适用了编译器的强类型规则。但在泛型类里面，输出字符串 kv.value.ClassName 则可以适用于所有类型，因为组件的Name或者TButton类的任何方法或属性我们都可以在源码中使用。
...
这样修改后，我们就可以把任何对象当成值的字段加以储存了。然而，我们就不能对解析出来的对象作太多动作，除非我们先把该对象转型成特定的类型。为了找到一个比较好的平衡点，我们可能会想要在特定的按钮跟任何对象之间找个介于中间的类型，例如说让储存的值只允许视觉组件:
...
对应的源码在 KeyValueGeneric 范例项目里面也有。最后，我们也可以建立一个储存一般数据的键-值对类的实体，不让它储存任何对象，而是储存整数值:
...
当我们宣告了一个泛型类型的变量，这个宣告可能写很长。当我们用这个类型建立一个对象时，我们也必须重复相同的宣告，除非我们运用行内变量宣告与这功能的便利，让该变量的类型可以直接被推定，上一个范例源码的宣告就可以改写为:
...
在这段源码里面，我们就不用重复写两次完整的泛型类型声明，对我们需要使用容器类的时候方便许多，我们稍后再介绍。


### 泛型的类型规则

当我们宣告了一个泛型类型的实体，这个类型就固定下来了，同时在后续的所有动作中，就会被编译器视为确定类型，所以如果我们有个泛型类是像这样的:
...
当我们为这个类型声明特定的对象时，我们在 Value 字段就不能指派跟宣告时不同类型的数据了。以下面两个对象为例，以下的部份指派就是不合法的(以下节录自 TypeCompRules 范例项目):
...
一旦我们在泛型的宣告中确定了特定的类型，编译器就已经确定了要检查的类型，我们的源码就必须要遵循 Object Pascal 的强类型规则了。类型检查也会把泛型对象当成一个特殊的类型。当我们把泛型类指定了特定的类型，我们就不能把指定另一个类型的泛型类对象指派到不同泛型类的变量了。只看文字说明很容易昏头，还是看一下源码，比较容易厘清内容:
...
我们接下来会在”泛型类型兼容性规则”这一节里面用明确的案例，指出类型兼容性规则是以结构检查，而不是以类型名称检查。我们不能在泛型类型已经确认了特定类型之后，又指派一个不同的、不相容的类型给一个泛型类型。


## Object	Pascal 里面的泛型

在前一个范例里，我们已经看过了如何在 Object Pascal 里面定义、使用泛型类型。我决定在深入到技术概念前，先用范例来介绍这个功能。因为泛型的技术概念相当复杂，也相当重要。从编程语言的观点看完泛型之后，我们再回头多看几个范例，包含了定义、使用泛型容器类，这是 Object Pascal 里面主要使用这个技术的地方之一。

我们已经介绍过，当我们定义一个类的时候，可以加入额外的参数，使用尖括号把特定的类型括起来即可:
...
泛型类型可以用来当做数据字段的类型(就像我们在前一个例子里面介绍的)、当做属性的类型、或者作为参数的类型，甚至是一个函数的回传类型，还可以用在更多地方。请注意，对一个区域字段(或者数组)使用这个类型并不是一种义务，有些情境下，泛型类型只用在作为回传值、参数，不能用在类的宣告上，但可以用来定义类的某些方法。

这种形式的延伸或泛型类型的宣告，不只是能对类使用，也能用在记录上面(我们在第五章介绍过，记录也可以有方法、属性、覆写的运算方法)。泛型类也可以有多个参数化的类型，我们用下面的源码作范例，让大家可以了解怎么在泛型的宣告中用上两个类型当成参数，一个当成方法的参数，另一个则是回传值的类型:
...
Object Pascal 里面对泛型的实作，跟其他静态编程语言一样，并不是基于执行时期的支持。这个功能是由编译器跟链接程序来处理的，在运行时间的机制几乎完全没有作什么事。跟虚拟函数的调用不同，虚拟函数是在执行时期进行绑定，而泛型类的方法则是在我们定义出完整的泛型类型实体时就被建立了，而且是在编译时建立的！我们将会看到这个功能可能的缺陷，但正面来看这个问题，它让我们知道泛型类比一般类的效率来的好，甚至连执行时期的检查所需的动作都省下了。在我们深入看到这些问题前，我们先来看些很明确的规则，这些规则打破了传统 Pascal 语言的类型兼容规则。


### 泛型类型兼容性规则

在传统的 Pascal 语言跟 Object Pascal 的核心类型兼容性规则都是基于类型名称是否一致。换句话说，两个变量的类型如果兼容，这两个类型的名称一定是一样的，不管两个类型里面实际上是怎么储存数据或定义方法的。

以下是一个静态数组间类型不兼容的传统例子(节录自 TypeCompRules 范例项目):
...
我们可以从上面的源码看到，四个数组在结构上是完全相同的。然而编译器只会让我们对类型兼容的数组进行指派。兼容的规则是必须类型名称完全相同(两个变量的类型都是 TArrayof10)，或者根本就是同一个类型(array3, array4 直接是宣告在同一行变量的宣告上)。

这个类型兼容性规则是很有局限性的例外，就像衍生类的相关性。这个规则的另一个例外，而且是很明显的例外，就是泛型类型的类型兼容性，这个情境也只会发生在编译器内部使用上，编译器会自行决定是否要位这个泛型类型建立一个新的内部类型，套用在该类型所有出现的地方。

新规则会界定泛型类型如果使用的是同样的泛别类，且在定义时使用同样的实体类型，就视为相同的类型，不管在定义时，类型的名称是否相同。换句话说，泛型类型实体的全名会是泛型类型跟实体类型的组合。

在以下的范例中，四个变量就是完全类型兼容的：
...
使用泛型类型来定义类是最常见的情境，泛型类型也可以用在非泛型的类中。换句话说，一般的类也可以有泛型方法。在这个情形下，我们不用在泛型符号出现的位置指定特定的类型，等到调用时再处理即可。以下就是一个简单的泛型方法的例子，节录自 GenericMethod 范例项目:
...
我们没办法在类似的类方法内部中作太多处理(至少在我们使用特定限制之前，我们会在下一章里面介绍)，所以我用特殊的泛型函数(稍后会介绍)写了一些源码来把这个类型转换成字符串，我们来看一下:
...
我们可以看到这个方法没有使用任何实际的值当做参数，只要求一些类型信息。在还不知道 T1 的类型之前，让写这段程序的时候相对变得很复杂。我们可以用以下各种参数来调用这个『全局泛型函数』，可以使用的版本非常多:
...
以上所有调用这个函数的写法都是正确的，参数化的类型在这些调用的源码里面是暗中传递的。注意到泛型类型是被明确指定的，但参数的类型并没有被指定，所以我们会看到以下的执行结果:
...
如果我们在调用这个方法的时候没有在尖括号中指定类型，实际的类型就会从该参数的类型来指定。如果我们用一个类型、一个参数来调用这个方法，参数的类型就必须跟泛型类型的宣告一致。所以以下三行就无法被编译了:
...
因为一些优化的例外，每次我们在为泛型类型实体化的时候，不管是泛型方法或是泛型类，编译器都会产生一个新的类型。这个新的类型在同一个泛型类型的不同实体(或者是同一个方法的不同版本)之间是不会共享源码的。

我们来看一个范例(节录自 GenericCodeGen 范例项目)。在这个程序中定义了一个泛型类，定义如下:
...
这三个方法的实作源码如下(请注意到 One 方法是绝对跟泛型类型独立的):
...
这个主程序当中使用了泛型类型要把它的方法被实体化时(这个动作会由编译器来处理)内存内的地址厘清。以下是它的源码:
...
如我所预期的，编译器不只在内存里面为每一种数据类型的 SetT 方法都建立了不同的版本，就连 One 方法也是如此，尽管事实上各版本的 One 方法都是一模一样的。

不仅如此，如果我们重复宣告了一个完全相同的泛型类型，我们也会得到另一组新的实作函数。同样的，一个泛型类型的相同实作类型，在不同的单元文件里面被使用时，也会强迫编译器一再产生相同的源码，而可能会导致建立出的执行档明显变得很大。为了这个原因，如果我们要建立一个内含很多个方法的泛型类，而这些方法跟泛型的类型没有什么关系的话，我会建议把这些方法定义在一个非泛型的类里面，然后以这个非泛型的类衍生一个需要使用到泛型类型的泛型类：透过这个方法来实作，基础类的方法就只会被编译器产出一次，执行档也就不会太大了。
...
目前我们在看泛型类型定义的最大问题，就是我们能对泛型类类型的元素所做的非常少。我们可以用两个科技来降低这个限制。第一项就是使用在 RTL 里面特别用来支持泛型的特殊函数。第二项(也更有效)则是在泛型类可以使用的类上面作限制。

这一节我们将聚焦在第一项上面，下一节再来介绍使用上的限制。我们刚提到过，在 RTL 里面有一些函数是可以用来对泛型类型定义中的参数化类型进行处理的:

+ Default(T)是一个和泛型功能一起出现的新函数，如果我们传入了任何一个非泛型的类型，它会回传空值，或 0，或 null，回传值可以是 0、空字符串、nil 等等。以 0 清除过的内存对同一种类型的全局变量也会有一样的值(跟局部变量不同，事实上，全局变量会被编译器以 0 作初始化)
+ TypeInfo(T)回传目前这个泛型型态在运行时间的指针，我们会在第十六章里面介绍更多关于类型信息的相关数据。
+ SizeOf(T)会以 Byte 数回传该类型里面所使用的内存大小(如果 T 是对象或是字符串的话，这个回传值就是每个内存地址的大小，在 32 位系统中会回传 4 bytes, 64 位则回传 8 bytes)。
+ IsManagedType(T)会告诉我们该类型在内存中是否是受管理的，如果是字符串或者动态数组的话，回传值就会是 true.
+ HasWeakRef(T)，这个函数是跟支持 ARC 的编译器相关的，会回传一个内存参考是否为弱参考，需要特定的内存管理支持。
+ GetTypeKind(T)：是让我们能够从类型信息中取得类型种类的快捷方式，这个回传值比 TypeInfo 所回传的类型定义来的更高阶。

!!! note "笔记"
    以上这些方法所回传的，都是编译器处理过的常数，而不是在运行时间中实际调用什么函数所响应的值。重要的不是这些处理很快，而是让编译器跟链接程序可以对产出的源码进行优化，把没有使用到的源码移除。如果我们打算用这些函数的回传值来进行判断，编译器会发现判断式(if..else)其中的一段源码会被执行，而会自动移除掉没有被使用到的那一段。当相同的泛型方法被指定不同的类型，在编译时就会使用不同的判断状况，但要记得，源码都会先被产生，在优化的时候才有这些处理。

在GenericTypeFunc范例项目中有一个泛型类，会显示三个泛型类型函数:
...
在 GetDataName 方法中，我使用了 GetTypeName 函数(宣告在 TypeInfo 单元文件里面)，而不是直接存取数据结构，因为这样会从储存类型名称的已编码字符串进行适当的转换

因为有上述的宣告，我们可以把下列的测试源码进行编译，它会重复三次，使用三个不同的泛型类型实体。我就省略了重复的源码，只显示用来存取data 字段的程序，它们会依照实际类型来进行内容变更：
...
请注意，我们也可以对指定的类型使用泛型类型函数，在泛型类的内容之外，例如可以这样写:
...
_____________________________________________________________________
!!! note "笔记"
    我们不能直接对变量进行 TypeInfo 调用，例如上面的 TypeInfo(s)，这个调用的参数只能是一个类型。

### 泛型类的类建构函数

有个很有趣的例子，就是为泛型类定义一个类建构函数。事实上，这样的建构函数是由编译器所建立，并由每一个泛型类的实体所调用的，也就是说，每一个实际的类型被定义时，都使用了泛型样板。这一点就相当有趣了，因为这会让我们的程序在不使用类建构函数来建立每个泛型类实体的时候，初始化程序的源码变得相当复杂。

举个例子，想象一下内含一些类数据的泛型类。我们只会取得一个类数据的实体，共享于每一个泛型类实体上。如果我们需要为这个类数据进行初始化，指派一个初始值给它，我们就不能透过单元文件的初始区来做这个事情，因为我们并不知道这个类实际上会使用哪个类型来宣告泛型类。

以下是一个最简单的范例，范例中，泛型类里包含有类建构函数，用来为 DataSize 这个类数据字段进行初始化，节录自 GEnericClassCtor 范例项目:
...
接下来是这个泛型类建构函数的源码，用了一个内部的字符串列表(请看完整的实作区源码)来追踪哪一个类建构函数有被调用到:
...
这范例程序建立了，也使用了几个泛型类的实体并宣告了第三个数据类型，这地三个资料类型则会被链接程序移除掉:
...
如果我们要求程序显示 ListSequence 的内容，我们只会看到已经被初始化的类型名字:
...
然而，如果我们用相同的数据类型在不同的单元文件里面建立了泛型实体，链接程序就不会如预期中那样运作，我们会看到同一个类型有多个泛型类建构函数。

!!! note "笔记"
    类似的程序并不容易界定。为了避免重复进行初始化，我们可能会想要检查类建构函数是否已经被执行过。通常这个问题对于泛型类来说，是全面性的限制当中的一个，而链接程序并没有能力对这情形加以优化。

我在本范例的第二个单元文件当中加入了一个名为 Useless 的程序，当我们取消这些源码的注释状态时，编译器就会标明这些源码有问题，当初始的顺序如下:
...
我们已经看过，在泛型类中，我们能对泛型类型的值所做的处理很少。我们可以把它传过来传过去，或是对它进行我们已经介绍过的类型处理中的任何一项。

要能够对泛型类的型态作更确切的动作，我们通常必须给它一些守则才行。例如如果我们规定泛型类必须是一个类，编译器就会让我们透过它来使用 TObject 的方法。我们也可以进一步规范该类必须是类架构中的特定部分，或者一定要实作特定的接口，这样我们才可能透过泛型类的实体调用特定的方法。

### 类守则 (Class	Constraints)

最简单的守则就是我们要求该类型必须是一个类，我们可以这么写，来达到这个守则：
...
透过特定的类守则，我们可以指定只有哪些对象类型可以作为泛型类型。以下的源码就可以作为例子(节录自 ClassConstraint 范例项目):
...
指定这个限制的好处是什么?在泛型类方法中，我们现在已经可以使用TObject 的任何一个方法了，包含虚拟方法喔！以下就是 TSampleClass 泛型类的方法 One 的源码:
...
!!! note "笔记"
    这里我们有两个建议。第一个是 InstanceSize 会回传该对象的实际大小，跟我们刚用过的 SizeOf 泛型函数不同，SizeOf 会回传参考类型的大小。其次，要留意到我们如何使用 TObject 类的 ToString 方法。

我们可以执行个几次，玩玩看这个程序，看一下实际的效果，在源码中我们定义了几个不同的泛型类型实体，就像这段程序片段:
...
请注意到透过宣告一个具有客制化过的 ToString 方法的类，这个版本会在数据对象的类型确定的时候被调用，不管实际上是什么类型被指派给泛型类型。换句话说，如果我们建立了一个以 TButton 为类的泛型类实体：
...
我们可以把这个对象当成 TSampleClass<TButton>类型的对象，或者作为一个特定泛型类型的实体。在两种情形下，One 方法都会调用到 ToString 这个特别版本的方法:
...
然而，这跟大多数的记录并没有太多的不同(并没有共同的基础类)，所以这个宣告相对的没有太多的新功能。


### 特定的类守则

如果我们的泛型类需要跟特定的类子集一起运作(特定的类继承结构树)，我们可能会想要以特定的基础类进行泛型类守则，例如我们可能会想要这样宣告:
...
这个泛型类的实体只容许组件类，也就是 TComponent 的任一个衍生类。这让我们的泛型类变得很确切(对，听起来很奇怪，但它的确很奇怪)，而且编译器会让我们在这个泛型类的实体当中使用 TComponent 的所有方法。

如果这看起来很棒，再多想一下。如果我们考虑到要达成继承的目标与类型兼容性规则，我们可能会指出使用传统面向对象技术的相同问题，而不会使用泛型类了。我不是说特定的类守则完全没用，但它显然不如高阶的类守则或(我发现这个还蛮有趣的)以接口为基础的守则那么强大。



### 接口守则

不限制泛型类只能使用特定的类，改以限制只能使用有实作特定接口的类，可以透过参数类型的开放性来达成泛型类的通用性。这使得我们可以在泛型赢别的实体当中调用特定接口的方法。在泛型上使用接口守则在 C#编程语言是很常见的。我们用一个例子作为开场白(节录自IntfConstraint 范例项目)。首先，我们得先宣告一个接口：
...
为了表现这个泛型类的弹性，我为这个接口建立了另一个完全不同的实作方法:
...
我们在以下的源码里面，用类函数(没有在本书中列出)为 Parent 控件建立了一个组件，其位置随机指定：
...
在刚刚最后一个范例中，我定义了一个泛型类，可以接受有实作特定接口的任何类作为其实作类。我们也可以透过建立一个基于接口参考的标准类(非泛型类)来达到相似的效果。事实上，我们可以定义一个类，像这样(节录自 IntfConstraint 范例项目):
...
这两个作法之间的差别是什么?首先，在上述的类里面我们可以传递两个不同类型的对象给设定方法(setter)，提供的类都实作了特定的接口。但在泛型版本里面，我们只能够传递一种特定类型的对象(要看该泛型类实体要求哪一种类型)。所以泛型版本的程序在类型检查这一点，比较保守，限制也比较多。

从我的观点来看，主要的不同是使用接口为基础的版本表示当中有 Object Pascal 的参考计算器制在运作着，而使用泛型版本的时候，类会直接以特定类型的一般对象家里处理，因此参考计算器制并没有介入。再者，泛型版本可能有多重的限制，像是建构函数的守则，以及使用不同的泛型函数(像是位泛型类型要求特定的类型)，有些动作我们就不能做(事实上，当我们透过接口来处理，我们就不能使用基础类 TObject 的方法)。

换句话说，使用具有接口守则的泛型类，可以让这个类具有接口的好处，却没有接口的麻烦。但值得一提的是，这两种技术在大多数的情形来看都是完全一样的，且在其他情形下，这种作法则有更多的弹性。


### 预设建构函数守则

还有另一种可能的泛型类型守则，称为预设建构函数或者无需参数的建构函数。如果我们需要调用预设的建构函数来建立泛型类型的对象(例如要填满一个列表)，我们就可以使用这个守则。理论上(出处为官方说明文件)，编译器应该只让我们在具备预设建构函数的类型上使用它，如果没有预设的建构函数的话，编译器会跳过它，直接调用 TObject 的预设建构函数。一个具备建构函数守则的泛型类可以用以下的方式来写(节录自IntfConstraint 范例项目)：
...
!!! note "笔记"
    我们也可以指定不具备类守则的建构函数守则，在建构函数处理的类型会是一个类。把两者都列出来，会让程序更具备可读性。

假设用以下的宣告，我们可以使用建构函数来建立一个泛型内部对象，而不用知道该类实际上的运作方法：
...
我们要怎么使用这个泛型类，且实际的规则是什么?在下一个范例里面，我们提供了两个类。第一个是有预设建构函数的(无需参数)，第二个则是要求一个参数的建构函数：
...
就像前面提过的，理论上我们应该只能使用第一个类，但实务上我们却是两个都能使用:
...
!!! note "笔记"
    即使直接调用 TParamConst.Create 在编译时期会犯错(如上例所示)。类似的调用如果使用类参考或者其他间接方式调用就会成功，这或许也说明了建构函数守则效果的规则。

### 泛型守则的总整理以及组合应用

我们可以在泛型类型上应用许多中不同的守则，在此我们做个简单的整理，直接用源码来进行吧:
...
看过这些守则(我也花了不少时间来熟悉它们)之后，没办法很直觉的知道我们可以把这些守则进行组合。例如我们可以定义一个泛型类，让它局限在特定类的子类架构下并且要求它具备特定的接口，像是:
...
并不是所有的组合都有意义：例如我们不可以同时要求使用类跟记录，在使用类守则时，合并使用特定的类守则是多余的。最后，请注意方法守则则是一个特例，它可以达成单一方法接口守则的要求(然而比实际上体验的更复杂)。


## 预先定义的泛型容器


因为在 C++语言发展样板的早期，最常被使用的样板类已经被定义在样板容器或者整个清单里面了，对这些已定义的类，C++语言则是把他们定义为一个标准样板函数库(Standard Template Library, 简称 STL)。当我们在定义一个对象的列表时，就像在 Object Pascal 里面提供的TObjectList，我们就拥有了一个可以储存任何类型对象的列表。使用继承或者组合的方式，我们都需要定义一个特殊类型的自定容器，但这会是个乏味(其中也潜藏危机)的功能。

Object Pascal 编译器原本就内建了一些泛型容器的类，我们可以在Generics.Collections 单元文件里面找到。当中的四个核心容器类都是独立实作的(彼此之间没有继承关系)，这些类都是以类似的现代方法实作的(使用动态数组)，而且也都对应到相应的非泛型容器类中，原来的容器类都放在 Contnrs 单元文件里面:
...
这些类逻辑上的差异，从它们的命名上应该可以看出明显的不同。要熟悉跟测试它们最好的方法，是在原本使用非泛型容器类的源码跟使用不同数据类型的泛型版本，比较看看有多少差别。

!!! note "笔记"
接下来要讨论的这个程序，ListDemoMd2005，只使用了一些方法，所以在泛型与非泛型的类当中，并没有对接口兼容性做太多的测试，不过我决定用一个已存在的程序，不要重新写一个。另一个要使用这个范例的原因，则是我们可能在原有的源码里面进行泛型修改的时候，可以透过泛型这个功能而获得额外的改进。


### 使用 TList<T>
名为 ListDemoMd2005 的这个程序里面，有一个用来定义 TDate 类的单元文件，而主窗体里面就使用了一个 TList 来储存日期数据。在程序一开始的 uses 区段，我加入了 Generics.Collection 的引用，然后把主窗体里面的宣告改成了这样:
...
新的日期列表在类型检查上比原本通用的指标列表更能自动的对类型进行检查。移除了上面这一行有问题的源码以后，程序就可以正常编译、正常运作了。不过，它还有改进的空间。
...
请注意当中的类型转换，因为这个程序使用了指针型的列表(TList)，而不是组件列表(TObjectList)。
...
就像前面提到的 TList<T>这个泛型类有较高的兼容性。它包含了所有原有的方法，像是 Add, Insert, Remove, 以及 IndexOf。同时也提供了 Capacity跟 Count 属性。只是 Items 变成了 Item，而且是默认属性(可以直接用变量名称加上方括号来存取，不用透过属性名称)，过去我们不常直接用这种方式存取。
...
反之，我们要做的是为 IComparer<T>定义一个适当的实作方法。为了让类型能兼容，我们得定义能够针对 TDate 类进行排序的实作源码。有很多个方法可以完成这个目标，包含使用匿名方法(我们虽然在下一章才会介绍到匿名方法，不过下一节会先介绍作法)。这是个有趣的技术，也是因为它让 我们有机 会 可 以 介 绍 许多种泛型的设计模式。这是透过 位 于Generics.Defaults 单元文件的部分结构化类，名为 TComparer 来达成的。


!!! note "笔记"
    我之所以把这个类称为结构化类，是因为它协助了源码结构化，而没有在实作上加入一大堆多余的名词。类似的类通常也被称为框架类，通常也用在较为复杂的设计上。

这个类是定义成抽象化类，并以泛型实作当中的接口：
...
我们所要做的，是先以特定的数据类型(例如本例中的 TDate)为泛型类实体化，也继承一个实作了为这个数据类型处理的 Compare 方法的类。这两个动作可以一次完成，我们透过以下的程序来说明一下:
...
如果你觉得这段程序看起来不太适应，那很正常。新类继承自泛型类的特定实体，我们可以从以下两个独立的步骤来体验:
...
这两个独立的宣告应该可以精简需要建立的源码，我们只要在宣告TAnyDateComparer 类型的单元文件里面宣告它们即可。

我们可以在原始码里面找到 Compare 函数的实际源码，不过因为它不是这一节的关键，所以我就不在这里特别贴出来了。但是要记得，即使我们把列表排序过了，用 IndexOf 方法也不会比较有效率(这一点跟 TStringList 类完全不同喔)
...
TList<T>类的 Sort 方法的参数 IComparer<T>，事实上可以被用来调用TComparer<T>的 Construct 方法，会把一个匿名方法当做参数传递，可以写成:
...
!!! note "笔记"
    上例中的 DoCompare 方法是以类似匿名方法的原理运作的，即使没有函数名称也能运作。我们稍后会以另一个程序片段来说明不需要函数名称的这个事实。我们到下一章会介绍 Object Pascal 这个新功能的更多信息。也请留意到，我们为 TDate 记录也定义了两个比较用的运算符号，可以让源码稍微简化，但即使有类的存在，比对的源码也必须放在该类的方法里面才行。
...
这个例子应该有激发你对于匿名方法学习的欲望吧!肯定的是，最后这个例子写起来比前一节里面的源码简单的多，虽然对许多 Object Pascal 开发人员来说建立一个衍生类看起来已经很简洁易懂了(继承的版本会把逻辑区隔的很清楚，让未来可能重复使用源码时相对简单的多，但许多时候，我们仍然还是不会去使用这个功能)。

### 对象容器(Object	Containers)

除了在这一节一开始介绍的泛型类，还有四个从定义在 Generics.Collections 单元中的基础类继承而来的泛型类，可以模仿现存在Contnrs(这是以容器类的基础)单元里面的类功能：
...
和它们的基础类相较之下，有两个关键性的差异。第一个是这些泛型类只能用在对象上，第二则是他定义了一个客制化的 Notification 方法，万一对象要从列表当中(除了选择性的调用OnNotify事件处理常数)被移除了，就会释放掉该对象。

换句话说，TObjectList<T>类的规则跟它的非泛型类 OwnsObjects 属性被设定成 true 的时候规则是一样的。如果您觉得对于为什么这个属性不再是可以被设定的感到好奇，可以回想一下 TList<T>也是一样，可以直接使用对象类型，跟它的非泛型类规则有些不同了。

还有第四个类 TObjectDictionary<TKey, TValue>，这个类则是以不同的方法定义的，它可以拥有 Key 对象，Value 对象，或者两者同时拥有。我们可以在类建构方法的 TDictionaryOwnerships 属性中看到所有可能性。

### 使用泛型字典(Dictionary)

在所有预先定义的泛型容器类中，Dictionary 或许是最值得我们花时间好好了解的了：TObjectDictionary<K, V>。

!!! note "笔记"
    Dictionary 在这个案例中，意指一种元素的集合，在这个集合当中，可以透过唯一键作为查询与写入的依据，Dictionary 从其作用上，也可以把它想成是关系型数组。在古典的 Dictionary 当中，我们只能用字符串当成查询的关键，但在程序实作的时候，键(Key)未必要是字符串(虽然字符串是比较常用的对象)。在旧版 Delphi 当中的通用集合中，Dictionary 的提示里使用了 TKey跟 TValue 的词语。然而 TValue 是不具关联的运行时间数据类型(我们会在第 16 章里面介绍)，在 Delphi 11 里面这两个词语则被改为 K 跟 V，以避免语意混淆。本版当中也已经做了相应的更新，如大家在前面的范例中可以发现的。

其他的类也很重要，但是它们似乎相对容易使用，也容易理解。为了示范 Dictionary 的使用，我写了一个范例，可以从数据表里面取出数据，为每个记录建立一个对象，然后使用综合索引值作为键值。这个作法的原因是相似的结构可以很容易就用来建立代理关系(proxy, 延迟初始化)，而这种关系里，键值可以当成从数据库加载数据时的轻量化数据版本。

以下是两个在 CustomerDictionary 范例项目中使用到的类，用以透过键与值的对应储存数据。第一个只有两个对应数据来呼应数据库的数据表，第二个则有比较复杂的数据结构(我已经省略了私有数据字段，取得数据的方法，以及设定数据的方法)：
...
第一个类非常单纯(每一个对象在被建立的时候就已经完成了初始化)，TCustomer类使用了延迟初始化(或者代理)模式且把一个参考记录在原始码数据库中，以对象变量的(class var)形式分享给所有对象。

!!! note "笔记"
    在这个范例中，我使用了一个衍生自 TDataSet 的对象来存取数据库的数据，好让它跟现实世界的情境比较贴近点。讨论在 Delphi 中存取数据库的确超越了本书的范围，所以我并不会提供额外的篇幅来描述 TDataSet 类实际上是如何作用的。

当对象被建立的时候，它的参考就已经被指派为对应的 TCustomerKey，而类数据则参照到源数据集合(dataset)。在每一个数据取得的方法中，该类会检查对应的对象是否的确已经完成了初始化，然后才进行回传：
...
透过这两个类，我在这个应用程序中加入了一个特殊用途的 dictionary。这个客制化的 dictionary 类继承自特定类型实体化的泛型类，并加入一个特定的方法:
...
!!! note "笔记"
你可能会想要用一个实际的数据集合来换掉 ClientDateSet 组件，把这个范例扩展成真的可以使用的程序。这样就可以透过查询一个键值和每一个实际的 TCustomer 对象的数据进行关连了。我有类似的源码，但是在这里介绍的话有点离题太远，目前这个范例只是一个实验性的泛型 dictionary 类而已。
...
经过了这么多年，许多 Object Pascal 的开发人员，包含我自己，都过度使用了 TStringList 类(译者:我也是)。我们不只可以用 TStringList 来储存字符串，还可以用来储存成对的名称与内容。不仅如此，还可以用来储存成对的名称与对象，更可以透过搜寻名称来取得对应的对象。从简单的介绍来看，字符串列表看起来比泛型好理解多了，简直就像个瑞士万用刀一样了。

特定、聚焦的容器类，的确相对是比较好的选择。举例来说，一般的TDictionary 类，以字符串当做键值，以对象当做内容，会比 TStringList来的好，至少有两个优点:程序清晰明白、也比较安全。因为当中会牵涉到的类型转换比较少，且执行速度比较快，这当然是假设该 dictionary 是使用哈希表(Hash table)的。
...
法则是会找出字符串列表中的对象，第二个方法会使用 dictionary。请注意，使用第一个方法时，我们需要用到 as 类型转换来变成特定的类型，dictionary则是已经跟类绑定了。以下是这两个方法的主要循环:
...
注意到其中和标准接口的差异，在泛型接口中，我们不需要定义明确的GUID 作为该接口的代号(或称 IID)。编译器会在我们为泛型接口进行实体化的时候主动建立一个 IID，也算是隐藏式的宣告。事实上，我们不用为泛型接口建立特定的实体也可以实作它，只要定义一个泛型类，让该类实作这个泛型接口即可：
...
在建构函数指派该对象的初始值的时候，解构函数的唯一要求是把该对象先记录下来注记成未来要释放的对象。我们要建立这个泛型类的实体(在背景建立一个接口类型的特定实体)，可以这么写：
...
我们在 IntfConstraint 范例项目里面看过一个替代的作法，是用一个特定类型的接口变量，然后明确的写明该接口类型的定义(不像一般我们在写类型时都是隐藏的写法):
...
当然，我们也可以定义一个特定的类来实作这个泛型接口，就像以下这个案例(节录自 GenericInterface 范例项目):
...
请留意，TGetValue<T>泛型类实作了 IGetValue<T>接口，TButtonValue特定类实作了 IGetValue<Integer>这个特定接口。在前面的例子当中，这个接口会被重新对应到组件的 Left 属性:
...
在上面的这个类中，MakeTButtonValue 这个类函数是一个已经准备好随时可以用来建立该类对象的方法。这个方法会在主窗体的地三个按钮里面被用上：
...
虽然这跟泛型类完全没关系，以下是 MakeTButtonValue 这个类函数的源码:
...
现在我们已经看过了如何定义泛型接口，以及如何把泛型接口跟泛型类或一般类结合的方法，我们再回头看一下 Generics.Default 单元文件。这个单元文件定义了两个泛型比较用的接口:

+ IComparer<T>拥有一个 Compare 方法
+ IEqualityComparer<T>则拥有 Equals 跟 GetHashCode 方法

这些接口则由一些泛型类或者一般类实作出来，兹列于下(实作的内容就不列了):
...
在上面的列表中，我们可以看到用于实作泛型接口的基础类要不是传统的参考计数功能的 TInterfacedObject 类，就是 TSingletonImplementation类(从 Delphi 11 开始，它只是 TNoRefCountObject 类的别名了)。这是一个名字很奇怪的类，因为它只提供了 IInterface 的基本实作，而且没有用到参考计数。

!!! note "笔记"
    singleton 这个名词一般是用来定义只能被建立出一个实体对象的类，而不是不具备参考计数功能的类。我觉得这个名字取得真的不恰当。

就像我们在本章『对 TList<T>进行排序』那一节所提到的，这些比较用的类会被泛型容器使用到。为了让程序能不要太复杂，Generics.Default 单元文件里面大量使用了匿名方法，所以您或许需要先看一下下一章再回来重新阅读一次。

## 在 Object	Pascal 里面的智能指标(Smart	Pointer)

开始接触泛型的时候，我们一开始可能会误以为 Object Pascal 大多数是用它来处理集合对象的。当然处理集合对象对泛型类来说是最基本的情境，在文件或者书籍里面的第一个范例也大多都是使用集合或者容器类来说明泛型。在本章的最后一个范例里，我们会介绍非集合类的泛型类，也就是定义一个智能指标(Smart Pointer)。
...
!!! note "笔记"
    在 OOP 里面，多型(polymorphism)这个名词是用来标注以下情形：我们把一个衍生类的对象指派给基础类的变量，并且调用基础类中的虚拟方法，最后可能会调用到特定子类所重新实作的版本。
...
请留意除了类守则外，泛型记录也有建构函数守则，因为我想要建立泛型数据类型的对象。这会发生在 GetValue 方法被调用，且该字段还没有被初始化的时候，以下是所有方法的完整源码:
...
然而，有鉴于泛型智能指标支持对指定类型对象自动建构的功能，我们也可以透过明确宣告该变量指向字符串列表的作法，用以下的源码来建立它:
...
要用接口来实作智能指针，我们可以写一个内部的支持类、让它跟接口绑定，并使用参考计数的机制来决定何时要释放对象。这个内部类的源码会像这样:
...
我把这个类当成泛型智能指标宣告成嵌套类型。我们需要在智能指标泛型类型上所做的，好让这个功能能够使用的，是新增一个接口参考，并用TFreeTheValue 对象进行初始化，以参照到内部对象：
...
!!! note "笔记"
    虽然可能会发生例外，但这段程序还是可以执行的。事实上，当我们使用到受管理的类型时，编译器还是会自动在编译时在这些源码中加入try-finally 区块。就像在刚刚这案例中，记录中使用了接口字段。
...
我们无法检查内部对象是否已经被指派，因为记录(跟类不同)并不会被初始化为 0。然而我们可以从接口变量来测试，接口变量会被初始化。或者我们也可以用额外的建构函数源码，把该记录用 0 全部填满作初始化。
...
## 以泛型处理协同变异(Covariant)回传类型

在 Object Pascal 当中(以及大多数静态面向对象编程语言)，一般来说一个方法可以回传一个类的对象，但我们不能把一个对象覆写成另一个衍生类之后，当成该类来回传。这个作法称为『协同变异回传类型』，在一些编程语言里面是有明确支持的，例如 C++。
...
在 Object Pascal 里面我们不能让虚拟方法回传不同的值，我们也不能对回传类型进行多载，只能在参数上进行多载。我们来透过简单的范例介绍完整的源码吧，以下是三个我们会用到的类:
...
这两个方法的实作源码非常简单，我们注意到类函数是实际上用来建立新组件的，它会从内部调用一个建构函数。

理由是我不想直接建立一个建构函数，这已经介绍过很多次了，建构函数是一个类用来建立对象的方法(或者类架构)。源码如下:
...
现在我们可以用以下的源码来使用这个类了，这不是我喜欢的用法，假设我们要把回传来的结果转成适当类型:
...
再次强调，我想做的是让TCat.Get的回传值能够被指派到TCat类型的参考，而不要多做一次转型，我们该怎么做呢?



### 回传泛型结果的方法

泛型可以协助我们解决这个问题。并不是泛型类型，泛型类型是最常用的泛型形式。但我们已经在本章前面的篇幅里面介绍过提供非泛型类型的泛型方法了。我们可以在 TAnimal 类中加入的是一个以泛型类型作为参数的方法：


`class function GetAs<T: class> (const aName: string): T;`

这个方法会要求一个泛型类型作为参数，在这个例子里泛型类型是个类(或实际的类型)，并回传该类型的物件。简单的实作源码如下:
...
现在我们可以建立一个实际的案例了，透过 as 转型，不过我们还是可以把类型当成参数传进去:
...
当我们回传相同类的对象时，我们可以直接改上面的源码来当做建构函数。但使用泛型来取得协同变异回传类型是更有弹性的。事实上我们可以用它来回传不同类的对象，甚至是不同架构的类：
...
!!! note "笔记"
    像这样的类，被用来为其他类(或者不只一个类，视其参数或者状态而定)建立对象，通常称为类工厂(class factory)。

我们现在可以使用这个特定的类守则(在该类来说，也许不可能达成)，透过确立建构函数守则，好让泛型方法可以用来建立特定的类对象：
